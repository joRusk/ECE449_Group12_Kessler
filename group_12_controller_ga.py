...
class Group12ControllerGA(KesslerController):
    ...
    def generate_chromosome(self):
        ...
        b = round(random.uniform(a + 0.01, 0.99), 2)
    ...
    def set_up_fuzzy_system(self, chromosome):
        ...
        closest_asteroid_ship_dist['F'] = fuzz.trimf(closest_asteroid_ship_dist.universe, [90, 1281, 1281])

        ship_turn = ctrl.Consequent(np.arange(-180, 180, 1), 'ship_turn')  # Degrees due to Kessler
        # Declare fuzzy sets for the ship_turn consequent; this will be returned as turn_rate.
        ship_turn['NL'] = fuzz.trimf(ship_turn.universe, [-180, 180 * chromosome[0].value[1],
                                                          180 * chromosome[0].value[2]])  # [-180,-180,-30]
        ship_turn['NS'] = fuzz.trimf(ship_turn.universe, [180 * chromosome[1].value[0], 180 * chromosome[1].value[1],
                                                          180 * chromosome[1].value[2]])  # [-90,-30,0]
        ship_turn['Z'] = fuzz.trimf(ship_turn.universe, [180 * chromosome[2].value[0], 180 * chromosome[2].value[1],
                                                         180 * chromosome[2].value[2]])  # [-30,0,30]
        ship_turn['PS'] = fuzz.trimf(ship_turn.universe, [180 * chromosome[3].value[0], 180 * chromosome[3].value[1],
                                                          180 * chromosome[3].value[2]])  # [0,30,90]
        ship_turn['PL'] = fuzz.trimf(ship_turn.universe,
                                     [180 * chromosome[4].value[0], 180 * chromosome[4].value[1], 180])  # [30,180,180]
        ...
        ship_fire['N'] = fuzz.trimf(ship_fire.universe,
                                    [-1, chromosome[5].value[0], chromosome[5].value[1]])  # [-1,-1,0.0]
        ship_fire['Y'] = fuzz.trimf(ship_fire.universe,
                                    [chromosome[6].value[0], chromosome[6].value[1], 1])  # [0.0,1,1]

        ship_thrust = ctrl.Consequent(np.arange(-500, 500, 1), 'ship_thrust')
        # Declare fuzzy sets for the ship_turn consequent; this will be returned as turn_rate.
        ship_thrust['RF'] = fuzz.trimf(ship_thrust.universe, [-500, round(500 * chromosome[7].value[1]), round(
            500 * chromosome[7].value[2])])  # [-500, -500, -250]
        ship_thrust['RS'] = fuzz.trimf(ship_thrust.universe,
                                       [round(500 * chromosome[8].value[0]), round(500 * chromosome[8].value[1]),
                                        round(500 * chromosome[8].value[2])])  # [-250, -100, 0]
        ship_thrust['FS'] = fuzz.trimf(ship_thrust.universe,
                                       [round(500 * chromosome[9].value[0]), round(500 * chromosome[9].value[1]),
                                        round(500 * chromosome[9].value[2])])  # [0, 100, 250]
        ship_thrust['FF'] = fuzz.trimf(ship_thrust.universe,
                                       [round(500 * chromosome[10].value[0]), round(500 * chromosome[10].value[1]),
                                        500])  # [250, 500, 500]
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool]:
        ...
        try:
            ...
            print("\nError during training; please try again")
            sys.exit()
    ...
    @property
    def name(self) -> str:
        return "Group 12 Controller GA"


if __name__ == "__main__":
    controller = Group12ControllerGA()
